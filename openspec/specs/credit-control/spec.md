...
## Purpose
Define the operator-facing balance management workspace for Sub2API users, including credit discovery, manual balance adjustment, automatic recharge policy execution, and local audit records.
...
- **THEN** the system reads user, group, API-key, and usage context from the local operational data snapshots when available
...
- **THEN** each item includes consumption fields for the selected usage window when usage data is available
...
- **WHEN** the operator filters by search text, user status, group, balance range, consumption range, usage segment, or usage window
...
- **THEN** the UI shows aggregate totals for visible users, including user count, total balance when known, total consumption for the selected window when known, and usage segment counts when segment records exist
...
- **THEN** the UI opens a detail panel with current balance, recent consumption, current group context, API key usage data when available, usage segment profile when available, and recent credit adjustment audit entries for that user
...
- **THEN** the system supports selected-group targeting when group context is available
...
The system SHALL always persist manual adjustment attempts, automatic policy changes, and automatic recharge run outcomes.
