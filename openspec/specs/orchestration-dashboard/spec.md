...
- **THEN** each item includes `flow_id`, `email`, `user_id`, `group_id`, `assignment_mode`, `status`, `account_name`, `oauth_account_id`, `error_message`, `created_at`, and `updated_at`
...
#### Scenario: Start flow records orchestration progress
- **GIVEN** the operator starts a provisioning flow with a valid email
- **WHEN** the system creates the flow, provisions the user and group assignment, binds the user to the group, and generates the OAuth URL
- **THEN** the system persists timeline events for the start request, user creation, group assignment resolution, user group binding, OAuth URL generation, and pending OAuth handoff
