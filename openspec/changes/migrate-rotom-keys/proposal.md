...
## Summary
Add an authenticated orchestration action that scans the admin user's API keys, detects keys named in the `service:object:version:email` style, and transfers each key to the exact matching existing Sub2API user while preserving the key string.
