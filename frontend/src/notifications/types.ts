...
export const notificationSignalGroups: NotificationSignalGroup[] = [
  {
    ...
    signals: [
      ...
      { key: "proxy_unreachable", label: "代理不可达", description: "定时探活判死的代理（连通/延迟/关键目标不达标），按代理逐个触发与恢复。", source: "proxy health", defaultThreshold: "1", defaultOperator: "gte", defaultSeverity: "critical", defaultReadIntervalMinutes: 1 },
      { key: "proxy_all_down", label: "代理全部不可用", description: "所有代理同时判死，账号已回落直连；持续触发直到有代理恢复。", source: "proxy health", defaultThreshold: "1", defaultOperator: "gte", defaultSeverity: "critical", defaultReadIntervalMinutes: 1 },
      { key: "account_evicted", label: "账号被驱逐", description: "健康巡检暂停了该账号的调度（限流自动回归，需人工类修复后回归），按账号逐个触发与恢复。", source: "account health", defaultThreshold: "1", defaultOperator: "gte", defaultSeverity: "warning", defaultReadIntervalMinutes: 1 },
